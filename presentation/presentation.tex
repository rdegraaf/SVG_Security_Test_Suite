...
\begin{frame}[fragile]{Same-Origin Policy}{The traditional security model for the web}
  \begin{itemize}
    \item Something needs to go here...
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Content Security Policy}{An example}
  ...
  \begin{itemize}[<+->]
    ...
    \item Styles are only allowed from external files at the same origin.
    \item Scripts are only allowed from external files at the same origin, and from other origins over HTTPS.
    \item Static images are allowed from from files at the same origin, \texttt{data:} URIs, and from files at \texttt{*.svg.test}.
    \item Objects and frames are allowed from the same origin and from \texttt{http://nocsp.svg.test}.
    \item Media, fonts, and connections are not allowed on any origin.
  \end{itemize}
\end{frame}
