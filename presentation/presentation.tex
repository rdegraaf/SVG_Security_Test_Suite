...
\date[iSEC Open Forum]{11 December 2014}
\titlegraphic{\includegraphics[height=1.5cm]{images/iseclogo}}
...
% Defined after the title page so that the logo doesn't appear there
\logo{\includegraphics[height=0.8cm]{images/iseclogo.pdf}}

\begin{frame}{Outline}
  TODO: Test suite details
  TODO: New vulns?
  TODO: Firefox
...
\begin{frame}[fragile]{Same-Origin Policy}
  \begin{itemize}
    ...
    \item Scripts running inside a document from one origin can only interact with documents from the same origin.
    \item \url{http://www.example.come/somedir/page.html} has the same origin as \url{http://www.example.com/otherdir/doc.html}, but not the same origin as \url{https://www.example.com/somedir/page2.html} or \url{http://en.example.com}.
...
\begin{frame}[fragile]{Content Security Policy}{Source lists}
  \begin{itemize}
    \item Each directive must have a source list.  Sources can be
  \end{itemize}
  \begin{description}
    \item['none'] Content covered by the directive must not be allowed from any source.
    \item['self'] The origin of the document to which CSP applies.
...
\begin{frame}[fragile]{Content Security Policy}{An example}
  ...
  \begin{itemize}[<+->]
    ...
    \item Styles are only allowed from external files at the same source.
    \item Scripts are only allowed from external files at the same source, and from other source over HTTPS.
    \item Static images are allowed from from files at the same source, \texttt{data:} URIs, and from files at \texttt{*.svg.test}.
    \item Objects and frames are allowed from the same source and from \texttt{http://nocsp.svg.test}.
...
\end{subfigure}
\end{figure}
{\tiny Chrome bug 384527\footnote{\url{https://code.google.com/p/chromium/issues/detail?id=384527}}; fixed in Chromium build 37.0.2054.0}
\end{frame}
...
\end{subfigure}
\end{figure}
Work-around: list the origin explicitly in \texttt{style-src} rather than relying on \texttt{'self';}
\end{frame}
...
\begin{frame}[fragile]{Other issues}
  \begin{itemize}
    ...
    \item Both Chrome\footnote{\url{https://code.google.com/p/chromium/issues/detail?id=378500}} and Firefox\footnote{\url{https://bugzilla.mozilla.org/show_bug.cgi?id=1018310}} display in-line SVG even under the CSP \texttt{default-src 'none';}.  There does not appear to be agreement on whether an in-line SVG is an image or nested document, or something else.  My position is that since \texttt{data:} URIs can be used to create in-line images or nested documents and can be blocked using CSP, in-line SVG should be blockable as well.
...
\begin{frame}[fragile]{SVG Security Test Suite}
  \begin{itemize}
    \item \url{https://github.com/rdegraaf/SVG}
    \item TODO: more information
  \end{itemize}
\end{frame}
